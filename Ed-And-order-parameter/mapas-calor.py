...
# plt.show()
# temp=[0.369,0.185,0.780]
# #temp=[0.3]
# for elem in temp:

#     te=elem
#     init=time.time()
#     data = pd.read_csv('1total_{}.csv'.format(te))

#     xx=list(data['interaction'])
#     yx=list(data['parametro'])
...
#     print("numero de elementos antes:",len(y),len(list(set(y))))
#     yasum1=list(np.load("total_y_{}.npy".format(te)))
#     # xasum1=list(np.load("total_x_{}.npy".format(te)))
#     yasum2=list(np.load("1total_y_{}.npy".format(te)))
#     # xasum2=list(np.load("total_x_{}.npy".format(te)))
#     yasum3=list(np.load("1total_y_{}.npy".format(te)))
...
#     plt.close()
    ...
    #     transform=ax.transAxes, ha='right', va='top')
    # plt.savefig("Free-energy-histogram-N=20-T={}.png".format(te))
    # plt.show()
    # plt.close()
...
for elem in temp:
    te=elem
    y=list(np.load("1total_y_{}.npy".format(te)))
    x=list(np.load("1total_x_{}.npy".format(te)))
    y_filtr=list(set(y))
    x_filtr=list(set(x))
    print(len(y_filtr))
    ...
    plt.xlabel('Interacion Energy', size=13)
    plt.ylabel('$q_{EA}$',size = 13)
    plt.colorbar()
    plt.show()
    FIG.savefig('mapa-calor-T={}.png'.format(te))
fini=time.time()
print(fini-init)